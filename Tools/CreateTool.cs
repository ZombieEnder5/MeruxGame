...
namespace Merux.Tools
{
	internal class CreateTool : ToolScript
	{
		...
		public override void Tick(float deltaTime)
		{
			...
			var vert = Merux.Game.EpsilonMaxBy(MathUtil.GetPartFaceNormalPairs(contact.part).ToArray(), v => v.Item1.Dot(contact.normal), 1e-2);
			Merux.Game.selector.Position = contact.point + Merux.Game.selector.CFrame.Rotation * (vert.Item2 * Merux.Game.selector.Size * .5);
		}
